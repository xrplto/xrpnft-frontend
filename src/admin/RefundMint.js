...
export default function RefundMint() {
    ...
    const [resolveRefund, setResolveRefund] = useState(null);
    ...
    useEffect(() => {
        function getRefunds() {
            ...
            axios.post(`${BASE_URL}/admin/refunds?page=${page}&limit=${rows}`, body, {headers: {'x-access-account': accountAdmin, 'x-access-token': accountToken}})
                .then(res => {
                    let ret = res.status === 200 ? res.data : undefined;
                    if (ret) {
                        setTotal(ret.total);
                        setRefunds(ret.refunds);
                    }
                }).catch(err => {
                    console.log("Error on getting refunds list!!!", err);
                }).then(function () {
                    // always executed
                    setLoading(false);
                });
    ...
    const onResolveRefund = async (refund) => {
        if (!accountAdmin || !accountToken) {
            openSnackbar('Please login', 'error');
            return;
        }
        ...
        try {
            ...
            const res = await axios.post(`${BASE_URL}/admin/resolve_refund`, body, {headers: {'x-access-account': accountAdmin, 'x-access-token': accountToken}});
    ...
    const handleResolve = (refund, action) => {
        refund.action = action;
        setResolveRefund(refund);
        setOpenConfirm(true);
    }
    ...
    return (
        <>
            ...
            >
                ...
                <ToggleButton value="notrefunded" sx={{pl:2, pr:2, pt: 0.3, pb: 0.3}} style={{textTransform: 'none'}}>Not Refunded</ToggleButton>
                                            ...
                                            <Stack spacing={0.5}>
                                                <Stack direction="row" spacing={0.2} alignItems="center">
                                                    <Stack direction="row" spacing={1} alignItems="center">
                                                        ...
                                                        <Typography variant="s8">{src} {srcName && <Typography variant="s2">({srcName})</Typography>}</Typography>
                                                    </Stack>
                                                    <Link
                                                        underline="none"
                                                        color="inherit"
                                                        target="_blank"
                                                        href={`https://bithomp.com/explorer/${src}`}
                                                    ...
                                                    <CopyToClipboard text={src} onCopy={()=>openSnackbar('Copied!', 'success')}>
                                                ...
                                                <Stack direction="row" spacing={0.2} alignItems="center">
                                                    <Stack direction="row" spacing={1} alignItems="center">
                                                        <Typography variant="s7">Who: </Typography>
